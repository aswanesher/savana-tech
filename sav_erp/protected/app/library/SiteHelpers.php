...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	//	echo '<pre>'; print_r($forms);echo '</pre>';
		...
		{		
			...
			if($format == 'grid')
			{
				...
				$active = ($i==0 ? 'active' : '');
				$f .= '<div class="tab-pane padding-lg '.$active.'" id="'.trim(str_replace(" ","",$tit)).'"> 
				';			
			}	
	...
	public static  function transForm( $field, $forms = array(),$bulk=false , $value ='')
	{
		...
		switch($type)
		{
			...
			case 'text';			
				$form = "<input  type='text' name='".$field."{$bulk}' class='form-control input-sm' value='{$value}'/>";
				break;

			case 'text_date';
				$form = "<input  type='text' name='$field{$bulk}' class='date form-control input-sm' value='{$value}'/> ";
				break;
			...
			case 'select';
				if($option['opt_type'] =='external')
				{
					...
					foreach($data as $row):
						...
						$opts .= "<option $selected value='".$row->$option['lookup_key']."' > ".$row->$option['lookup_value']." </option> ";
					endforeach;
				...
				$form = "<select name='$field{$bulk}'  class='form-control'>
			...
			case 'radio';
				...
				for($i=0; $i<count($opt);$i++) 
				{
					$checked = '';
					$row =  explode(":",$opt[$i]);
					$opts .= "<option value value ='".ltrim(rtrim($row[0]))."' > ".$row[1]." </option> ";
				}
				$form = "<select name='$field{$bulk}' class='select2' ><option value=''> -- Select  -- </option>$opts</select>";
				break;												
	...
	public static function gridDisplay($val , $field, $arr) {
		...
		{
			$Q = DB::select(" SELECT ".$arr['display']." FROM ".$arr['db']." WHERE ".$arr['key']." = '".$val."' ");
			if(count($Q) >= 1 )
			{
				$row = $Q[0];
				return $row->$arr['display'];
			} else {
				return '';
			}
		} else {
	...
	public static function gridDisplayView($val , $field, $arr) {
		...
		{
			$Q = DB::select(" SELECT ".$arr['3']." FROM ".$arr['1']." WHERE ".$arr['2']." = '".$val."' ");
			if(count($Q) >= 1 )
			{
				$row = $Q[0];
				return $row->$arr['3'];
	...
	{
		$avatar = '<img alt="" src="http://www.gravatar.com/avatar/'.md5(Session::get('email')).'">';
		...
		if($row->avatar !='' ) 	
		{
			...
			{
				return  '<img src="'.URL::to('uploads/users').'/'.$row->avatar.'" border="0" width="'.$width.'" />';
			} else {
				return $avatar;
			}	
		} else {
