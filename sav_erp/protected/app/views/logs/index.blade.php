  ...
  <div class="page-content">
    ...
    <div class="breadcrumb-line">
	  ...
	  <ul class="breadcrumb-buttons collapse">
			...
			<li><a href="{{ URL::to('logs/download') }}" class="tips" title="{{ Lang::get('core.btn_download') }}">
			<i class="icon-folder-download2"></i>&nbsp; </a></li>	
			@endif		
		 	@if(Session::get('gid') ==1)
			<li><a href="{{ URL::to('module/config/logs') }}" class="tips"  title="{{ Lang::get('core.btn_config') }}">
			<i class="icon-cog"></i>&nbsp; </a></li>	
			@endif  
			@if($access['is_add'] ==1)
	   		<li><a href="{{ URL::to('logs/add') }}" class="tips"  title="{{ Lang::get('core.btn_create') }}">
        ...
        <tbody>
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <td>
				 	<div class="btn-group">
					{{--*/ $id = SiteHelpers::encryptID($row->auditID) /*--}}
				 	@if($access['is_detail'] ==1)
					<a href="{{ URL::to('logs/show/'.$id)}}"  class="tips btn btn-xs btn-default"  title="{{ Lang::get('core.btn_view') }}"><i class="icon-paragraph-justify"></i> </a>
					@endif
					@if($access['is_edit'] ==1)
					<a  href="{{ URL::to('logs/add/'.$id)}}"  class="tips btn btn-xs btn-success"  title="{{ Lang::get('core.btn_edit') }}"> <i class="icon-pencil4"></i></a>
					@endif
