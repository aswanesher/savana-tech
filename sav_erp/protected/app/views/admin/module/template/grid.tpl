  ...
  <div class="page-content">
    ...
    <div class="breadcrumb-line">
	  ...
	  <ul class="breadcrumb-buttons collapse">
			...
			<li><a href="{{ URL::to('{class}/download') }}" class="tips" title="{{ Lang::get('core.btn_download') }}">
			<i class="icon-folder-download2"></i>&nbsp; </a></li>	
			@endif		
		 	@if(Session::get('gid') ==1)
			<li><a href="{{ URL::to('module/config/{class}') }}" class="tips"  title="{{ Lang::get('core.btn_config') }}">
			<i class="icon-cog"></i>&nbsp; </a></li>	
			@endif  
			@if($access['is_add'] ==1)
	   		<li><a href="{{ URL::to('{class}/add') }}" class="tips"  title="{{ Lang::get('core.btn_create') }}">
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <td>
					...
					<a href="{{ URL::to('{class}/show/'.$id)}}"  class="tips btn btn-xs btn-default"  title="{{ Lang::get('core.btn_view') }}"><i class="icon-paragraph-justify"></i> </a>
					@endif
					@if($access['is_edit'] ==1)
					<a  href="{{ URL::to('{class}/add/'.$id)}}"  class="tips btn btn-xs btn-success"  title="{{ Lang::get('core.btn_edit') }}"> <i class="icon-pencil4"></i></a>
					@endif
