		<div class="col-md-7 offset-0" style="margin-top: 10px;">
			<?PHP
				...
				$qryCekTiketHoliday		= $conn->query("SELECT flag_hari_libur FROM m_tiket WHERE hotel_id = '".$strDataTerakhir['hotel_id']."'");
				...
				if ($sekarang == 6 || $sekarang == 7 || $strHargaTiketHoliday['flag_hari_libur'] == 1){
					...
					$qryHargaWeekEnd= $conn->query("SELECT tiket_nama,tiket_harga_dewasa_libur,tiket_harga_anak_libur FROM m_tiket WHERE hotel_id = '".$strDataTerakhir['hotel_id']."'");
					...
					$qryHargaWeekEnd= $conn->query("SELECT tiket_nama,tiket_harga_dewasa_holiday,tiket_harga_anak_holiday FROM m_tiket WHERE hotel_id = '".$strDataTerakhir['hotel_id']."'");
					$strHargaWeekEnd= $qryHargaWeekEnd->fetch(PDO::FETCH_ASSOC);
					$tiketNama		= $strHargaWeekEnd['tiket_nama'];
					...
					$qryHargaWeekEnd= $conn->query("SELECT tiket_nama,tiket_harga_dewasa_biasa,tiket_harga_anak_biasa FROM m_tiket WHERE hotel_id = '".$strDataTerakhir['hotel_id']."'");
					$strHargaWeekEnd= $qryHargaWeekEnd->fetch(PDO::FETCH_ASSOC);
					$tiketNama		= $strHargaWeekEnd['tiket_nama'];
			...
			<table border="1" width="100%">
				...
				<?PHP
					...
					$qryMenuMasakan	= "SELECT b.tiket_nama,b.tiket_harga_permainan FROM guest_book_wisata_detail a INNER JOIN m_tiket b ON a.tiket_id = b.tiket_id WHERE a.guest_book_wisata_detail_kode = '".$strDataTerakhir['guest_book_wisata_detail_kode']."'";
