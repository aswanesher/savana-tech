  ...
  <body id="top" class="thebg" onload="load()" >
	...
	<div class="container">
		...
		<div class="container pagecontainer offset-0">
			<table width="100%">
				...
				$tglIn = $tgl1[2]."-".$tgl1[0]."-".$tgl1[1];
				  ...
				  $q = $conn->query("SELECT * FROM kalender_harga WHERE cal_bulan = ".$tgl1[0]." and cal_tahun= ".$tgl1[2]."  and room_id=".$room_id."");
				...
				<tr>
					...
					<td style="border-bottom: 2px solid #ebebeb;" align="center">
						<select name="noroom" style="width: 50%" onchange="slcRoom(this.value)" >
							<?php 
								for($a=1;$a<=$row_room['room_avaibility'];$a++){
									?>
					...
					<span class="green size15"><b>Rp. <?PHP echo number_format($totStlhDisc,2,",",".");?></b></span><br>
					...
					//VALIDATION FORM
					if ($datein != '' && $dateout != ''){
						if($row_room['room_avaibility']==0){
					?>
						<button class="bookbtn mt1b" disabled width="30px" ><b>Habis</b></button>
					<?PHP
					}else {
					?>
		...
		<div class="container pagecontainer offset-0">
			...
			?>
			<?PHP
				$qryFac2 	= "SELECT b.fac_nama FROM m_room_detail a INNER JOIN m_facilities b ON a.fac_id = b.fac_id WHERE a.hotel_id = '".$id."'";
				$stmtFac2 	= $conn->prepare( $qryFac2 );
				$stmtFac2->execute();
				while ($rowFac2 = $stmtFac2->fetch(PDO::FETCH_ASSOC)){
				extract($rowFac2);
			?>
			<div class="col-md-3">
				<div class="hpadding23">
