<?PHP
	...
	//JIKA PEMESANAN DENGAN SUPIR
	if(isset($_SESSION['spr']) && $_SESSION['spr'] == 1){
		$qryCekHargaSupir 	= $conn->query("SELECT rent_harga_supir FROM m_hotel WHERE hotel_id = '".$carid."'");
		$dataHargaSupir		= $qryCekHargaSupir->fetch(PDO::FETCH_ASSOC);
		$hargaSupir			= $dataHargaSupir['rent_harga_supir'];
		$totyar				= $totyar + $hargaSupir;
	}else{
		$totyar				= $totyar;
	}
